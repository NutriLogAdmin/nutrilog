...
const COLUMNS = 'title, badge, color, intro, steps, tip, is_public'

// Recetas propias + las que otros usuarios han marcado como compartidas
router.get('/', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT r.id, r.title, r.badge, r.color, r.intro, r.steps, r.tip, r.is_public,
...
router.post('/import', async (req, res) => {
  ...
  try {
    ...
    for (const r of clean) {
      await pool.query(`
        INSERT INTO recipes (user_id, ${COLUMNS}) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
      `, [req.user.id, r.title, r.badge, r.color, r.intro, r.steps, r.tip, r.is_public])
    }
...
router.post('/', async (req, res) => {
  ...
  try {
    const result = await pool.query(`
      INSERT INTO recipes (user_id, ${COLUMNS}) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id
    `, [req.user.id, r.title, r.badge, r.color, r.intro, r.steps, r.tip, r.is_public])
...
router.put('/:id', async (req, res) => {
  ...
  try {
    await pool.query(`
      UPDATE recipes SET title=$1, badge=$2, color=$3, intro=$4, steps=$5, tip=$6, is_public=$7
      WHERE id=$8 AND user_id=$9
    `, [r.title, r.badge, r.color, r.intro, r.steps, r.tip, r.is_public, req.params.id, req.user.id])
    res.json({ ok: true })
  } catch (err) {
